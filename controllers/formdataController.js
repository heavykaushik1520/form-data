...
//   let formDataObj = await createFormData(formData)
//   if (!formDataObj){
...
// async function createFormData(formData, res) {
//   try {
//     if (!formData) {
//       return res.status(400).json({ message: "Form data is required" });
//     }
...
//     if (!newForm) {
//       return res.status(400).json({ message: "Error in creating form data" });
//     }
//     return ({newForm
//     });
//   } catch (error) {
...
async function createFormData(formData) { // Removed the 'res' parameter
  try {
    if (!formData) {
